...
bool MockLink::_connect(void)
{
    if (!_connected) {
        ...
        mavlink_status_t* incomingStatus = mavlink_get_channel_status(_mavlinkIncomingChannel);
...
bool MockLink::_incomingAcceptUnsignedCallback(const mavlink_status_t* status, uint32_t message_id)
{
    Q_UNUSED(status);

    // Find the MockLink this callback is associated with
    MockLink* mockLink = nullptr;
    auto linkManager = qgcApp()->toolbox()->linkManager();
    QList<SharedLinkInterfacePtr> sharedLinks = linkManager->links();
    for (auto& sharedLink: sharedLinks) {
        mockLink = qobject_cast<MockLink*>(sharedLink.get());
        break;
    }

    if (status->signing->link_id == mockLink->_mavlinkIncomingChannel && mockLink->isSecureConnection()) {
        // Secure connections accept all messages
        return true;
    } else {
        // Link is not secure so we deny unsigned messages
        if (!mockLink->_denyingUnsignedMessageReported) {
            qCWarning(MockLinkLog) << Q_FUNC_INFO << "Unsigned message received on insecure connection - accept denied";
            mockLink->_denyingUnsignedMessageReported = true;
            mockLink->denyingUnsignedMessage();
        }
        return false;
    }
}
...
bool MockLink::_allocateMavlinkChannel()
{
    ...
    if (!signingKeyBytes.isEmpty()) {
        if (MAVLinkSigning::initSigning(static_cast<mavlink_channel_t>(_mavlinkIncomingChannel), signingKeyBytes, MockLink::_incomingAcceptUnsignedCallback)) {
            qCDebug(MockLinkLog) << "Signing enabled on incoming channel" << _mavlinkIncomingChannel;
        } else {
    ...
    qCDebug(MockLinkLog) << "_allocateMavlinkChannel outgoing:incoming" << mavlinkChannel() << _mavlinkIncomingChannel;
